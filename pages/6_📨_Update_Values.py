...
def render_page():
    ...
    choice = st.sidebar.radio('Update', options=['WOs & Quotes', 'MP Status', 'Main Process', 'OTIF', 'Last WOs Date', 'Quoted Items'], label_visibility='collapsed')
...
def update_quoted_items():
    ...
    if st.button('Update Items 👌🏼'):
        errors = automations.update_item_quotations(verbose=True)

        st.success('Quoted items have been updated')
        show_errors(errors=errors)
